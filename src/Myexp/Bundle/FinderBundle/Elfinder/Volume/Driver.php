...
namespace Myexp\Bundle\FinderBundle\Elfinder\Volume;
...
abstract class Driver {
    ...
    public function mount(array $opts) {
        ...
        if ($root['read']) {
            ...
            if ($this->options['startPath']) {
                ...
                if (!empty($start) && $start['mime'] == 'directory' && $start['read'] && empty($start['hidden']) && $this->inpathCE($this->options['startPath'], $this->root)) {
    ...
    public function isUploadableByName($name, $allowUnknown = true) {
        $mimeByName = Driver::mimetypeInternalDetect($name);
        return (($allowUnknown && $mimeByName === 'unknown') || $this->allowPutMime($mimeByName));
    }

    /**
     * Return Extention/MIME Table (Driver::$mimetypes)
     * 
     * @return array
     * @author Naoki Sawada
     */
    public function getMimeTable() {
        return Driver::$mimetypes;
    }
    ...
    protected function mimetype($path, $name = '') {
        ...
        if ($this->mimeDetect == 'finfo') {
            if ($type = @finfo_file($this->finfo, $path)) {
                if ($ext && preg_match('~^application/(?:octet-stream|(?:x-)?zip)~', $type)) {
                    if (isset(Driver::$mimetypes[$ext]))
                        $type = Driver::$mimetypes[$ext];
            ...
            $type = Driver::mimetypeInternalDetect($path);
        }
        ...
        return $type == 'unknown' && $this->mimeDetect != 'internal' ? Driver::mimetypeInternalDetect($path) : $type;
    }
    ...
    static protected function mimetypeInternalDetect($path) {
        ...
        if (!Driver::$mimetypesLoaded) {
            Driver::$mimetypesLoaded = true;
            $file = dirname(__FILE__) . DIRECTORY_SEPARATOR . 'mime.types';
            if (is_readable($file)) {
                $mimecf = file($file);
                foreach ($mimecf as $line_num => $line) {
                    if (!preg_match('/^\s*#/', $line)) {
                        $mime = preg_split('/\s+/', $line, -1, PREG_SPLIT_NO_EMPTY);
                        for ($i = 1, $size = count($mime); $i < $size; $i++) {
                            if (!isset(Driver::$mimetypes[$mime[$i]])) {
                                Driver::$mimetypes[$mime[$i]] = $mime[0];
                            }
                        }
                    }
                }
        ...
        return isset(Driver::$mimetypes[$ext]) ? Driver::$mimetypes[$ext] : 'unknown';
    }
    ...
    protected function countSize($path) {
        ...
        $result = 0;
        foreach ($this->getScandir($path) as $stat) {
            $size = $stat['mime'] == 'directory' && $stat['read'] ? $this->countSize($this->joinPathCE($path, $stat['name'])) : (isset($stat['size']) ? intval($stat['size']) : 0);
            if ($size > 0) {
                $result += $size;
            }
        }
    ...
    protected function copy($src, $dst, $name) {
        ...
        return $this->convEncOut($this->_copy($this->convEncIn($src), $this->convEncIn($dst), $this->convEncIn($name))) ? $this->joinPathCE($dst, $name) : $this->setError(elFinder::ERROR_COPY, $this->path($srcStat['hash']));
    }
    ...
    protected function copyFrom($volume, $src, $destination, $name) {
        ...
        if ($source['mime'] == 'directory') {
            ...
            // MIME check
            $stat = $this->stat($path);
            $mimeByName = Driver::mimetypeInternalDetect($stat['name']);
    ...
    protected function canCreateTmb($path, $stat, $checkTmbPath = true) {
        return (!$checkTmbPath || $this->tmbPathWritable) && (!$this->tmbPath || strpos($path, $this->tmbPath) === false) // do not create thumnbnail for thumnbnail
