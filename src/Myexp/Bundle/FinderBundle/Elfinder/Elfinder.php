...
namespace Myexp\Bundle\FinderBundle\Elfinder;
...
class Elfinder {
    ...
    public function __construct($opts) {
        ...
        foreach ($opts['roots'] as $i => $o) {
            ...
            $class = '\\Myexp\\Bundle\\FinderBundle\\Elfinder\\Volume\\'. $class;
    ...
    protected function fsock_get_contents(&$url, $timeout, $redirect_max, $ua, $outfp) {
        ...
        while (!$fp && $connect_try_count < $connect_try) {
            ...
            $errstr = "";
            $fp = @ fsockopen(
                            $arr['https'] . $arr['host'], $arr['port'], $errno, $errstr, $connect_timeout);
            if ($fp)
                break;
            $connect_try_count++;
